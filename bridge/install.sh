...
#   curl -fsSL https://raw.githubusercontent.com/MacSiem/openpets/main/bridge/install.sh | less

set -euo pipefail

REPO="https://github.com/MacSiem/openpets.git"
...
cat <<MSG
  ...
  All set. Bridge controls are in OpenPets.app's tray menu — click the 🐾
  icon, then "Bridge" submenu. There you'll find: status, start/stop, open
  config, open log, list pet packs, clear bubbles.

  From the terminal:

      openpets-bridge status                # health check
      openpets-bridge list-pets             # discover installed pet packs
      openpets-bridge clear --done-only     # tidy up done bubbles
